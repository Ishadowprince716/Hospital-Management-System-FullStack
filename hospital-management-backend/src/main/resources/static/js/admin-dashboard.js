...
document.addEventListener('DOMContentLoaded', () => {
    ...
    loadDashboardData();
    loadAdminInsights(); // Load AI Insights on start
    setupEventListeners();
...
async function loadAdminInsights() {
    const loading = document.getElementById('aiOpsLoading');
    const content = document.getElementById('aiOpsContent');
    ...
    try {
        ...
        renderAdminInsights(apiResponse.data);
    } catch (error) {
        console.error('AI Admin Insight Error:', error);
        content.innerHTML = `
            <p class="empty-state" style="grid-column: span 3; color: var(--danger);">
                <i class="fas fa-exclamation-circle"></i> Failed to load AI insights. Check your AI service configuration.
            </p>
        `;
    } finally {
...
function renderAdminInsights(insights) {
    ...
    content.innerHTML = `
        <div class="insight-block">
            <h4 style="color: var(--gray-700); margin-bottom: 0.75rem; display: flex; align-items: center; gap: 0.5rem;">
                <i class="fas fa-chart-line" style="color: var(--primary-color);"></i> Revenue Risk
            </h4>
            <div style="background: white; padding: 1rem; border-radius: 0.75rem; border: 1px solid var(--gray-200); height: 100%;">
                <p style="font-size: 0.9rem; line-height: 1.5; color: var(--gray-600); margin: 0;">${insights.revenueRiskSummary}</p>
            </div>
        </div>

        <div class="insight-block">
            <h4 style="color: var(--gray-700); margin-bottom: 0.75rem; display: flex; align-items: center; gap: 0.5rem;">
                <i class="fas fa-exclamation-triangle" style="color: #f59e0b;"></i> Bottlenecks
            </h4>
            <div style="display: flex; flex-direction: column; gap: 0.5rem;">
                ${insights.bottleneckAlerts.map(alert => `
                    <div style="background: rgba(245, 158, 11, 0.05); padding: 0.6rem 0.75rem; border-radius: 0.5rem; border-left: 3px solid #f59e0b; font-size: 0.85rem; font-weight: 600; color: #92400e;">
                        ${alert}
                    </div>
                `).join('')}
            </div>
        ...
        <div class="insight-block">
            <h4 style="color: var(--gray-700); margin-bottom: 0.75rem; display: flex; align-items: center; gap: 0.5rem;">
                <i class="fas fa-lightbulb" style="color: #10b981;"></i> Strategic Actions
            </h4>
            <ul style="margin: 0; padding-left: 1.2rem; display: flex; flex-direction: column; gap: 0.4rem;">
                ${insights.strategicRecommendations.map(action => `
                    <li style="font-size: 0.85rem; color: var(--gray-600);">${action}</li>
                `).join('')}
            </ul>
        </div>
    `;
...
window.loadAdminInsights = loadAdminInsights;
    ...
    try {
        ...
        try {
            ...
            if (appointmentsResponse.ok) {
                ...
                if (window.animateNumber) {
                    animateNumber(document.getElementById('totalAppointments'), appointments.length);
                } else {
                    document.getElementById('totalAppointments').textContent = appointments.length;
                }
            }
        } catch (e) {
            // Silently fail if appointments can't be loaded
        }
