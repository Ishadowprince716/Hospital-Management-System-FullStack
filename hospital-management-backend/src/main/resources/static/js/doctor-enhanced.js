    ...
    document.querySelectorAll('.emr-tab').forEach(tab => {
        tab.addEventListener('click', () => {
            ...
            if (tabName === 'ai-insights') {
                document.getElementById('emrContent').style.display = 'none';
                document.getElementById('aiInsightsContent').style.display = 'block';
                fetchAIInsights(currentPatientId);
            } else {
                document.getElementById('emrContent').style.display = 'block';
...
// Fetch AI Insights
async function fetchAIInsights(patientId) {
    const loading = document.getElementById('aiInsightsLoading');
    const dataContainer = document.getElementById('aiInsightsData');
    
    loading.style.display = 'block';
    dataContainer.innerHTML = '';

    try {
        const response = await fetch(`${API_BASE}/ai/health-summary/${patientId}`, {
            headers: {
        ...
        renderAIInsights(apiResponse.data);
    } catch (error) {
        console.error('AI Insights Error:', error);
        dataContainer.innerHTML = `
            <div class="alert alert-danger">
                <i class="fas fa-exclamation-circle"></i> Failed to generate AI insights. Please try again.
            </div>
        `;
    } finally {
...
function renderAIInsights(insights) {
    const container = document.getElementById('aiInsightsData');
    
    let alertsHtml = '';
    if (insights.criticalAlerts && insights.criticalAlerts.length > 0) {
        alertsHtml = `
            <div class="insight-section alerts">
                <h4><i class="fas fa-exclamation-triangle"></i> Critical Alerts</h4>
                <ul>
                    ${insights.criticalAlerts.map(alert => `<li class="alert-item-critical">${alert}</li>`).join('')}
                </ul>
            </div>
        `;
    }
        ...
        <div class="ai-insights-dashboard">
            <div class="insight-summary-card">
                <div class="insight-header">
                    <i class="fas fa-brain ai-icon-pulse"></i>
                    <h3>Clinical Summary</h3>
                </div>
                <p class="summary-text">${insights.recentHistorySummary}</p>
            </div>
            ...
            <div class="insights-grid-2">
                <div class="insight-section">
                    ...
                    <div class="med-chips">
                        ${insights.activeMedications.map(med => `<span class="med-chip">${med}</span>`).join('')}
                    </div>
                </div>
                ...
                <div class="insight-section">
                    ...
                    <ul class="action-list">
                        ${insights.recommendedActions.map(action => `<li><i class="fas fa-arrow-right"></i> ${action}</li>`).join('')}
                    </ul>
                </div>
            </div>
            
            <div class="ai-disclaimer-small">
                <i class="fas fa-info-circle"></i> AI-generated insights should be verified against the full patient medical record.
            </div>
        </div>
    `;
}
...
// EMR Tab Management
async function loadEMRTab(tabName) {
    const content = document.getElementById('emrContent');
    if (!content) return;

    try {
        const response = await fetch(`${API_BASE}/emr/patient/${currentPatientId}/complete`);
        const emrData = await response.json();
        ...
        switch (tabName) {
            case 'overview':
                ...
                break;
            case 'vitals':
                displayVitalsTab(emrData.vitalSigns);
                break;
            case 'history':
                displayHistoryTab(emrData.patient.medicalHistory);
                break;
            case 'allergies':
                displayAllergiesTab(emrData.allergies);
                break;
            case 'medications':
                displayMedicationsTab(emrData.prescriptions);
                break;
        }
    } catch (error) {
        console.error('Error loading EMR tab:', error);
    }
}

function displayVitalsTab(vitals) {
    const content = document.getElementById('emrContent');
    if (!vitals || vitals.length === 0) {
        content.innerHTML = '<p class="empty-state">No vital signs recorded</p>';
        return;
    }
...
function displayHistoryTab(history) {
    const content = document.getElementById('emrContent');
    content.innerHTML = `
        <div class="card" style="padding: 1.5rem;">
            <h3>Medical History</h3>
            <p style="white-space: pre-wrap; margin-top: 1rem;">${history || 'No medical history recorded'}</p>
        </div>
    `;
}

function displayAllergiesTab(allergies) {
    const content = document.getElementById('emrContent');
    if (!allergies || allergies.length === 0) {
        content.innerHTML = '<p class="empty-state">No allergies recorded</p>';
        return;
    }
...
function displayMedicationsTab(prescriptions) {
    const content = document.getElementById('emrContent');
    if (!prescriptions || prescriptions.length === 0) {
        content.innerHTML = '<p class="empty-state">No current medications</p>';
        return;
    }

    content.innerHTML = prescriptions.map(p => `
        <div class="prescription-summary" style="margin-bottom: 1.5rem; border-bottom: 1px solid #eee; padding-bottom: 1rem;">
            <h4>${p.diagnosis} <small>(${new Date(p.prescriptionDate).toLocaleDateString()})</small></h4>
            <ul style="margin-top: 0.5rem;">
                ${p.items?.map(i => `<li>${i.medicineName} - ${i.dosage} for ${i.duration}</li>`).join('') || '<li>No items recorded</li>'}
            </ul>
        </div>
    `).join('');
}
...
function renderAIInsights(insights) {
    ...
    if (insights.criticalAlerts && insights.criticalAlerts.length > 0) {
        alertsHtml = `
            <div class="insight-section alerts" style="background: rgba(239, 68, 68, 0.05); border: 1px solid rgba(239, 68, 68, 0.2); padding: 1rem; border-radius: 0.75rem; margin-bottom: 1.5rem;">
                <h4 style="color: #ef4444; margin-bottom: 0.5rem;"><i class="fas fa-exclamation-triangle"></i> Critical Alerts</h4>
                <ul style="list-style: none;">
                    ${insights.criticalAlerts.map(alert => `<li style="padding: 0.25rem 0; color: #b91c1c; font-weight: 600;">• ${alert}</li>`).join('')}
                </ul>
            </div>
        `;
    }
    ...
    container.innerHTML = `
        <div class="ai-insights-container">
            <div class="insight-summary" style="background: var(--gray-50); padding: 1.5rem; border-radius: 1rem; margin-bottom: 1.5rem;">
                <h3 style="display: flex; align-items: center; gap: 0.5rem; color: var(--primary-color);">
                    <i class="fas fa-brain"></i> AI Clinical Summary
                </h3>
                <p style="margin-top: 1rem; line-height: 1.6; font-size: 1.05rem;">${insights.recentHistorySummary}</p>
            </div>
            ...
            <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 1.5rem;">
                <div class="insight-section">
                    <h4 style="margin-bottom: 0.75rem;"><i class="fas fa-pills"></i> Active Medications</h4>
                    <div style="display: flex; flex-wrap: wrap; gap: 0.5rem;">
                        ${insights.activeMedications.map(med => `<span class="badge" style="background: #e0e7ff; color: #4338ca; padding: 0.5rem 0.75rem;">${med}</span>`).join('')}
                    </div>
                </div>

                <div class="insight-section">
                    <h4 style="margin-bottom: 0.75rem;"><i class="fas fa-clipboard-check"></i> Recommendations</h4>
                    <ul style="padding-left: 1.25rem;">
                        ${insights.recommendedActions.map(action => `<li style="margin-bottom: 0.5rem;">${action}</li>`).join('')}
                    </ul>
                </div>
            </div>
            
            <p style="margin-top: 2rem; font-size: 0.85rem; color: #666; font-style: italic; border-top: 1px solid #eee; padding-top: 1rem;">
                <i class="fas fa-info-circle"></i> This AI analysis is for clinical decision support and should be verified by a medical professional.
            </p>
        </div>
    `;
}
