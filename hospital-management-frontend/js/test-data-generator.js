...
async function populateTestData() {
    ...
    try {
        ...
        const doctors = await fetch(`${API_BASE}/doctors`, { headers: authHeaders }).then(r => r.json());
        const patients = await fetch(`${API_BASE}/patients`, { headers: authHeaders }).then(r => r.json());
        ...
        for (let i = 0; i < 8; i++) {
            ...
            try {
                const apt = await fetch(`${API_BASE}/appointments`, {
                    method: 'POST',
                    headers: authHeaders,
        ...
        const allAppointmentsRes = await fetch(`${API_BASE}/appointments`, { headers: authHeaders }).then(r => r.json());
        const allAppointments = allAppointmentsRes.data?.content || allAppointmentsRes.data || allAppointmentsRes;
        ...
        for (const apt of allAppointments) {
            if (apt.status === 'COMPLETED') {
                ...
                try {
                    ...
                    if (Math.random() > 0.4) {
                        await fetch(`${API_BASE}/bills/${bill.id}/payments`, {
                            method: 'POST',
                            headers: authHeaders,
                            body: JSON.stringify({
                                amount: amount,
                                paymentMethod: 'CASH',
                                notes: 'Paid during test generation'
                            })
                        });
                        billsPaid++;
